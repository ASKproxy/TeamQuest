...
public class MatchDetailFragment extends CommonFragment{
	...
	private void interestButtonSetUp()
	{
	 ...
	 private void savePlayerOpinion(RequestStatus status,boolean toRemove)
	    {
	    	CommonViewClass.showProgressdialog(getActivityObject());
	    	...
	    	MatchScheduleDM saveRequestStatusDm = new MatchScheduleDM();
				...
				saveRequestStatusDm.savePlayerOpinion(saveRequestStatus, Details.getPlayer(getActivityObject()).getPlayerId(), toRemove);
				if(toRemove)
				{
					...
					status.getPlayersList().add(Details.getPlayer(getActivityObject()));
				}
		...
		,final Button generalSaveOption,LinearLayout generalOptionDetailLayout)
{
	...
	protected void loadInBackGround() {
		...
		topicDetailList.clear();
		topicDetailList.addAll(topicDetails.getTopicDetailMatch(requestStatus.getUniqueId()));
		topicDetailSaveMap.clear();
