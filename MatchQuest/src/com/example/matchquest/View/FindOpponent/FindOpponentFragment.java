...
public class FindOpponentFragment extends CommonFragment{
	...
	private void loadAutoCompleteView()
	{
		...
		locationAdapter = new AutoCompleteAdapter(getActivity(), android.R.layout.simple_dropdown_item_1line,
				locAutoCompleteView,Arrays.asList(words));
...
//        };
	...
	public void saveInvite(RequestStatus opponentStatus) 
	{
    	...
    	int saveStatus = saveInvite.saveInvite(saveList.get(saveList.size()-1), opponentStatus);
    	...
    	try{
    		FindOpponentDM inviteAll = new FindOpponentDM();
				...
				case R.id.post_invite:
					...
					int postStatus =  inviteAll.postInvite(saveList);
					if(postStatus == 1)
					{
						Toast.makeText(getActivity(), "An invite has bee posted", 1000).show();
					}
	...
	protected void loadInBackGround() {
		try{
		searchByLoc = locAutoCompleteView.getText().toString();
	...
	public void updateView() {
		...
		searchStatus.setCreatedBy(Details.getPlayer(getActivityObject()).getPlayerId());
		searchStatus.setDate(searchByDate);
		searchStatus.setTime(searchByTime);
		searchStatus.setLocation(searchByLoc);
